...
func (y *YaagHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	Before(&SwaggerCall, r)
	y.nextHandler.ServeHTTP(writer, r)
	After(&SwaggerCall, writer, r)
}
...
func Before(apiCall *model.SwaggerSpec, req *http.Request) {
	requests := model.Paths{}
	requests.Path.Consumes = ReadHeaders(req)
	requests.RequestUrlParams = ReadQueryParams(req)
...
func After(apiCall *model.SwaggerSpec , record *responseRecorder, r *http.Request) {
